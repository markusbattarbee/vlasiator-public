...
/**
   ...
   plasma dynamics (such as the cross-shock potential, an electric field induced by electron pressure changes), we
   consider the electron pressure gradient term in the generalized Ohm's law:

   - nabla dot P_e /(n_e e)

   To model this, we can assume some equation of state for electrons. We now assume that the electron fluid has an
   isotropic pressure tensor (so pressure is a scalar), and that it is governed by a polytropic process, where P * V^k
   is constant. Here P is the pressure, V is the volume of an unit of fluid, and k is a polytropic index describing the
   process.

   Using this equation, for any position in the simulation, the electron temperature can be evaluated based on the local
   electron number density (assumed to be equal to the ion charge density fsgrids::moments::RHOQ divided by the
   elementary charge) and some normalization or anchoring values.
   ...
   some hypothetical anchor point, with values usually declared based on the upstream inflow conditions. These values
   are only used for normalizing the relationship between electron density and electron temperature for the given
   polytropic relation. These anchor point values are provided by the user as config parameters - the anchor point is
   not an actual physical point in the simulation, but just imagined for normalization purposes..
   ...
   that electron number density times elementary charge should match the ion charge density in the solar wind. A
   zeroth-order approximation is to set electron temperature to equal that of solar wind ions, but alternatively an
   observation-based factor (e.g. 0.25) may be applied to decrease electron temperatures at the anchor point.
   ...
   Thus, the gradient of electron pressure is calculated in derivatives.cpp using a standard slope limiter, for each
   Cartesian direction.

   Note:
   The value stored in e.g. dMomentsGrid.get(i,j,k)->at(fsgrids::dmoments::drhoqdx) is a difference, not a derivative.
   Thus, in the next step, evaluating the electron pressure gradient term in the general Ohm's law as E_gradPe(x,y,z) =
   - dPe/d(x,y,z) / (n_e * e * Delta(x,y,z)) requires adding EGradPeGrid.DX/DY/DZ in the denominator.

   Similar to the Hall term, we use the Parameters::hallMinimumRhoq value as a lower bound for electron number density
   in order to prevent runaway electric field terms at depletion zones.
...
void calculateEdgeGradPeTermXComponents(
   ...
   case 1:
      ...
      EGradPeGrid.get(i, j, k)->at(fsgrids::egradpe::EXGRADPE) =
          -dMomentsGrid.get(i, j, k)->at(fsgrids::dmoments::dPedx) / (limitedRhoq * EGradPeGrid.DX);
      break;
...
void calculateEdgeGradPeTermYComponents(
   ...
   case 1:
      ...
      EGradPeGrid.get(i, j, k)->at(fsgrids::egradpe::EYGRADPE) =
          -dMomentsGrid.get(i, j, k)->at(fsgrids::dmoments::dPedy) / (limitedRhoq * EGradPeGrid.DY);
      break;
...
void calculateEdgeGradPeTermZComponents(
   ...
   case 1:
      ...
      EGradPeGrid.get(i, j, k)->at(fsgrids::egradpe::EZGRADPE) =
          -dMomentsGrid.get(i, j, k)->at(fsgrids::dmoments::dPedz) / (limitedRhoq * EGradPeGrid.DZ);
      break;
