...
void calculateCurvature(FsGrid<std::array<Real, fsgrids::volfields::N_VOL>, FS_STENCIL_WIDTH>& volGrid,
   ...
   if (technicalGrid.get(i, j, k)->sysBoundaryFlag == sysboundarytype::NOT_SYSBOUNDARY &&
      ...
      vol->at(fsgrids::volfields::CURVATUREX) = bx * 0.5 * (rght_x_bx - left_x_bx) / technicalGrid.DX +
                                                by * 0.5 * (rght_y_bx - left_y_bx) / technicalGrid.DY +
                                                bz * 0.5 * (rght_z_bx - left_z_bx) / technicalGrid.DZ;
      vol->at(fsgrids::volfields::CURVATUREY) = bx * 0.5 * (rght_x_by - left_x_by) / technicalGrid.DX +
                                                by * 0.5 * (rght_y_by - left_y_by) / technicalGrid.DY +
                                                bz * 0.5 * (rght_z_by - left_z_by) / technicalGrid.DZ;
      vol->at(fsgrids::volfields::CURVATUREZ) = bx * 0.5 * (rght_x_bz - left_x_bz) / technicalGrid.DX +
