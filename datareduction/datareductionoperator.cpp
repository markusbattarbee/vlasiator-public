         ...
         arch::parallel_reduce<arch::sum>(
             ...
             ARCH_LOOP_LAMBDA(const uint i, const uint j, const uint k, const uint n, Real* lsum) {
                ...
                ARCH_INNER_BODY(i, j, k, n, lsum) {
                   ...
                   const Real VX =
                       block_parameters[BlockParams::VXCRD] + (i + HALF) * block_parameters[BlockParams::DVX];
                   const Real VY =
                       block_parameters[BlockParams::VYCRD] + (j + HALF) * block_parameters[BlockParams::DVY];
                   const Real VZ =
                       block_parameters[BlockParams::VZCRD] + (k + HALF) * block_parameters[BlockParams::DVZ];
                   // Compare the distance of the velocity cell from the center of the maxwellian distribution to the
                   // radius of the maxwellian distribution
                   if (((calculateNonthermal == true) &&
                        (((thermalV0 - VX) * (thermalV0 - VX) + (thermalV1 - VY) * (thermalV1 - VY) +
                          (thermalV2 - VZ) * (thermalV2 - VZ)) > thermalRadius * thermalRadius)) ||
         ...
         arch::parallel_reduce<arch::sum>(
             ...
             ARCH_LOOP_LAMBDA(const uint i, const uint j, const uint k, const uint n, Real* lsum) {
                ...
                ARCH_INNER_BODY(i, j, k, n, lsum) {
                   ...
                   const Real VZ =
                       block_parameters[BlockParams::VZCRD] + (k + HALF) * block_parameters[BlockParams::DVZ];
                   // Calculate the distance of the velocity cell from the center of the maxwellian distribution and
                   // compare it to the approximate radius of the maxwellian distribution
                   if (((calculateNonthermal == true) &&
                        (((thermalV0 - VX) * (thermalV0 - VX) + (thermalV1 - VY) * (thermalV1 - VY) +
                          (thermalV2 - VZ) * (thermalV2 - VZ)) > thermalRadius * thermalRadius)) ||
                       ((calculateNonthermal == false) &&
                        (((thermalV0 - VX) * (thermalV0 - VX) + (thermalV1 - VY) * (thermalV1 - VY) +
                          (thermalV2 - VZ) * (thermalV2 - VZ)) <= thermalRadius * thermalRadius))) {
                      // Add the value of the coordinates and multiply by the AVGS value of the velocity cell and the
                      // volume of the velocity cell
         ...
         arch::parallel_reduce<arch::sum>(
             ...
             ARCH_LOOP_LAMBDA(const uint i, const uint j, const uint k, const uint n, Real* lsum) {
                ...
                ARCH_INNER_BODY(i, j, k, n, lsum) {
                   ...
                   // compare it to the approximate radius of the maxwellian distribution
                   if (((calculateNonthermal == true) &&
                        (((thermalVX - VX) * (thermalVX - VX) + (thermalVY - VY) * (thermalVY - VY) +
                          (thermalVZ - VZ) * (thermalVZ - VZ)) > thermalRadius * thermalRadius)) ||
                       ((calculateNonthermal == false) &&
                        (((thermalVX - VX) * (thermalVX - VX) + (thermalVY - VY) * (thermalVY - VY) +
                          (thermalVZ - VZ) * (thermalVZ - VZ)) <= thermalRadius * thermalRadius))) {
                      lsum[0] += block_data[cellIndex(i, j, k)] * (VX - averageVX) * (VY - averageVY) * DV3;
      ...
      if (cell->get_number_of_velocity_blocks(popID) != 0)
         arch::parallel_reduce<arch::sum>(
             ...
             ARCH_LOOP_LAMBDA(const uint i, const uint j, const uint k, const uint n, Real* lsum)->void {
                ...
                int binNumber =
                    round((log(energy) - log(eminLocal)) / log(emaxLocal / eminLocal) * (nChannelsLocal - 1));
      ...
      if (cell->get_number_of_velocity_blocks(popID) != 0)
         arch::parallel_reduce<arch::sum>(
             ...
             ARCH_LOOP_LAMBDA(const uint i, const uint j, const uint k, const uint n, Real* lsum)->void {
                ...
                const Real mu =
                    Vpara / (normV + std::numeric_limits<Realf>::min()); // + min value to avoid division by 0
      ...
      if (cell->get_number_of_velocity_blocks(popID) != 0)
         arch::parallel_reduce<arch::sum>(
             ...
             ARCH_LOOP_LAMBDA(const uint i, const uint j, const uint k, const uint n, Real* lsum) {
                ...
                // We will use a gate function based on criteria that Vi-HALF*DVi <= BnormV[i] <= Vi+HALF*DVi (for
                // i=x,y,z or 0,1,2)
         ...
         arch::parallel_reduce<arch::sum>(
             ...
             ARCH_LOOP_LAMBDA(const uint i, const uint j, const uint k, const uint n, Real* lthread_epsilon_sum) {
                ...
                const Real bimaxwellian =
                    rho_local / sqrt(M_PI * M_PI * M_PI * V_par_th_sq * V_par_th_sq * V_par_th_sq) *
