...
bool writeVelocityDistributionData(const uint popID, Writer& vlsvWriter,
   ...
   try {
      ...
      for (size_t i = 0; i < cells.size(); ++i) {
         ...
         CHK_ERR(gpuMemcpy(&velocityBlockIds[blockIndex], GIDlist, nBlocks * sizeof(vmesh::GlobalID),
...
// Add a subarray to write. Note: We told beforehands that the vectorsize = WID3
      ...
      if (arrayElements > 0) {
         CHK_ERR(
             gpuMemcpy(arrayToWrite, SC->get_data(popID), arrayElements * WID3 * sizeof(Realf), gpuMemcpyDeviceToHost));
...
bool writeRestart(dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid,
                  ...
                  const bool dateInFileName, const int& stripe) {
   // Writes a restart
   bool success = true;
   int myRank;
