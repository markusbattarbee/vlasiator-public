...
int main(int argc, char* argv[])
{
	...
	*/
	...
	options.add_options()
		...
		("x_length",
			boost::program_options::value<uint64_t>(&x_length)->default_value(51),
			"Create a grid with arg number of unrefined cells in the x direction")
		("y_length",
			boost::program_options::value<uint64_t>(&y_length)->default_value(40),
			"Create a grid with arg number of unrefined cells in the y direction")
		("z_length",
			boost::program_options::value<uint64_t>(&z_length)->default_value(40),
	...
	float refine_L3radius =  6.371e7;
	float refine_L3nosexmin = 5.0e7;
	float refine_L3tailheight = 1.0e7;
	float refine_L3tailwidth = 5.0e7;
	float refine_L3tailxmin = -20.0e7;
	float refine_L3tailxmax = -5.0e7;

	float refine_L2radius = 9.5565e7;
	float refine_L2tailthick = 3.1855e7;
	float refine_L1radius = 1.59275e8;
	float refine_L1tailthick = 6.371e7;
	...
	if (refine_n > 0) {
	   for (const auto& cell: cells) {
		 ...
		 z = cell_center[2];
	      ...
	      // 			}
	      if ((sqrt(x * x + y * y + z * z) < refine_L1radius*refine_L1radius) ||
		  ((x < 0) && (std::abs(y) < refine_L1radius) && 
		   (std::abs(z)<refine_L1tailthick)))
		 {
		    grid.refine_completely(cell);
		 }		    
	   }
	...
	if (refine_n > 1) {
	   for (const auto& cell: cells) {
		 ...
		 z = cell_center[2];
	      ...
	      // 				grid.refine_completely(cell);
	      // 			}
	      if ((sqrt(x * x + y * y + z * z) < refine_L2radius*refine_L2radius) ||
		  ((x < 0) && (std::abs(y) < refine_L2radius) && 
		   (std::abs(z)<refine_L2tailthick)))
		 {
		    grid.refine_completely(cell);
		 }
	   }
	...
	if (refine_n > 2) {
	   for (const auto& cell: cells) {
		 ...
		 z = cell_center[2];
	      ...
	      // 				grid.refine_completely(cell);
	      // 			}
	      if ((x>refine_L3nosexmin) && (sqrt(x * x + y * y + z * z)<refine_L3radius*refine_L3radius))
		 {
		    grid.refine_completely(cell);			  
		 }
	      
	      // Check if cell is within the tail box
	      if ((x>refine_L3tailxmin) && (x<refine_L3tailxmax) &&
		  (abs(y)<refine_L3tailwidth) && (abs(z)<refine_L3tailheight))
		 {
		    grid.refine_completely(cell);
		 }
	   }
