...
#pragma omp parallel for schedule(guided, 1)
      for (size_t i = 0; i < cells.size(); ++i) {
         // easier to skip here than adding one more bool flag to calculateCellMoments - handles L2 outflow cells
         // without VDF
         if (mpiGrid[cells[i]]->sysBoundaryFlag == sysboundarytype::OUTFLOW &&
...
void balanceLoad(dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid, SysBoundary& sysBoundaries,
                 FsGrid<fsgrids::technical, FS_STENCIL_WIDTH>& technicalGrid, bool doTranslationLists) {
