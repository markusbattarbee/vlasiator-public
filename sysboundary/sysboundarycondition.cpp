...
void SysBoundaryCondition::vlasovBoundaryCopyFromTheClosestNbr(
    dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid, const CellID& cellID, const bool& copyMomentsOnly,
 ...
 * cells. \param mpiGrid Grid \param cellID The cell's ID. \param popID ID of the particle species. \param
 * copy_V_moments which set of moments to copy (_V or _R)
 */
void SysBoundaryCondition::vlasovBoundaryCopyFromAllClosestNbrs(
    dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid, const CellID& cellID, const uint popID,
 ...
 * cells. \param mpiGrid Grid \param cellID The cell's ID. \param popID ID of the particle species. \param
 * copy_V_moments which set of moments to copy (_V or _R) \patam fluffiness (parameter from 0 to 1; value 0 maintains
 * original value, 1 replaces it completely with average)
 */
void SysBoundaryCondition::vlasovBoundaryFluffyCopyFromAllCloseNbrs(
    dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid, const CellID& cellID, const uint popID,
    const bool copy_V_moments, creal fluffiness) {
 ...
 * \param fluffiness Factor to replace data with from 0.0 (default, do nothing) to 1.0 (replace all destination data
 * with source data)
 */
void averageCellData(dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid, const vector<CellID> cellList,
      ...
      if (mpiGrid[cellId]->sysBoundaryLayer == 2) {
         for (auto nbrPair : *mpiGrid.get_neighbors_of(cellId, Neighborhoods::SYSBOUNDARIES_EXTENDED)) {
            if (nbrPair.first != INVALID_CELLID) {
               ...
               if (mpiGrid[cellId]->sysBoundaryFlag == sysboundarytype::OUTFLOW &&
                   mpiGrid[neighbor]->sysBoundaryFlag == sysboundarytype::OUTFLOW &&
                   mpiGrid[neighbor]->sysBoundaryLayer == 1) {
                  // Find distance and update closestCells
                  d2L1 = nbrPair.second[0] * nbrPair.second[0] + nbrPair.second[1] * nbrPair.second[1] +
                         nbrPair.second[2] * nbrPair.second[2];
      ...
      for (int jj = -2; jj < 3; jj++) {
         for (int ii = -2; ii < 3; ii++) {
            if (technicalGrid.get(i + ii, j + jj, k + kk) // skip invalid cells returning NULL
                ...
                && technicalGrid.get(i + ii, j + jj, k + kk)->sysBoundaryFlag !=
                       sysboundarytype::OUTER_BOUNDARY_PADDING // Do not copy from there either
            ) {
               distance = min(distance, ii * ii + jj * jj + kk * kk);
            }
         }
...
vmesh::LocalID findMaxwellianBlocksToInitialize(const uint popID, spatial_cell::SpatialCell& cell, creal& rho, creal& T,
                                                creal& VX0, creal& VY0, creal& VZ0) {
   ...
   while (search) {
      if (0.1 * minValue >
              projects::MaxwellianPhaseSpaceDensity(counter * dV[0] + 0.5 * dvx, 0.5 * dvy, 0.5 * dvz, T, rho, mass) ||
