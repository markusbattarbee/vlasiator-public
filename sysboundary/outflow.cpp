...
#pragma omp parallel for schedule(static)
   for (uint i = 0; i < cells.size(); ++i) {
      ...
      if (doApply) {
         project.setCell(cell); // We set everything including VDF even in L2 cells to avoid a pile of spaghetti. Won't
                                // get communicated.
         ...
         case vlasovscheme::COPY:
            if (mpiGrid[cellID]->sysBoundaryLayer == 1) {
               vlasovBoundaryCopyFromTheClosestNbr(mpiGrid, cellID, false, popID,
                                                   calculate_V_moments); // copies VDF too
            } else {
...
#pragma omp for schedule(guided, 1)
      for (uint i = 0; i < cells.size(); i++) {
         ...
         // As of 20250505 the loop only does something for layer 1 in COPY mode so the check for layer 1 was moved here
         // for earlier loop continuation.
            ...
            for (uint i = 0; i < 6; i++) {
               if (isThisCellOnAFace[i] && facesToProcess[i] && !sP.facesToSkipVlasov[i]) {
                  ...
                  case vlasovscheme::NONE:
                     break;
                  case vlasovscheme::COPY:
                     // if(mpiGrid[cellID]->sysBoundaryLayer == 1) { // This is actually now (20250505) moved up a few
                     // lines for earlier loop continuation. Reinstate if other cases change!
                     vlasovBoundaryCopyFromTheClosestNbr(
                         mpiGrid, cellID, false, popID,
                         true); // first false means copy VDF too, second true means V moments
                     vlasovBoundaryCopyFromTheClosestNbr(
                         mpiGrid, cellID, false, popID,
                         false); // first false means copy VDF too, second false means R moments
                     // } // see comment above
                     break;
                  default:
                     abort_mpi("ERROR: invalid Outflow Vlasov scheme", 1);
...
#pragma omp single
      {
         SpatialCell::set_mpi_transfer_type(
             Transfer::ALL_SPATIAL_DATA); // No need to update VDFs, we only copy the VDFs from L1 to L2 below.
         mpiGrid.update_copies_of_remote_neighbors(Neighborhoods::SYSBOUNDARIES);
      }
...
#pragma omp for schedule(guided, 1)
      for (uint i = 0; i < cells.size(); i++) {
         const CellID cellID = cells[i];
         // As of 20250505 the loop only does something for layer 1 in COPY mode so the check for layer 2 was moved here
         // for earlier loop continuation.
            ...
            for (uint i = 0; i < 6; i++) {
               if (isThisCellOnAFace[i] && facesToProcess[i] && !sP.facesToSkipVlasov[i]) {
                  ...
                  case vlasovscheme::NONE:
                     break;
                  case vlasovscheme::COPY:
                     // if(mpiGrid[cellID]->sysBoundaryLayer == 2) { // This is actually now (20250505) moved up a few
                     // lines for earlier loop continuation. Reinstate if other cases change!
                     vlasovBoundaryCopyFromTheClosestL1OutflowNbr(
                         mpiGrid, cellID, true, popID,
                         true); // first true means copy moments only, second true means V moments
                     vlasovBoundaryCopyFromTheClosestL1OutflowNbr(
                         mpiGrid, cellID, true, popID,
                         false); // first true means copy moments only, second false means R moments
                     // } // see comment above
                     break;
                  default:
                     abort_mpi("ERROR: invalid Outflow Vlasov scheme", 1);
